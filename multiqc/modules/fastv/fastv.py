...
class MultiqcModule(BaseMultiqcModule):
    ...
    def __init__(self):
        ...
        super(MultiqcModule, self).__init__(name='fastv', anchor='fastv',
            href='https://github.com/OpenGene/fastv',
        ...
        self.fastv_coverage_plotdata = dict()
        self.fastv_mismatch_plotdata = dict()
        self.fastv_coverage_data = dict()
        for f in self.find_log_files('fastv', filehandles=True):
        ...
        if len(self.fastv_coverage_plotdata) > 0:
            self.add_section (
                name = 'Genome Coverage',
                anchor = 'fastv-genome-coverage',
                description = 'Genome coverage after filtering binsize: 10',
                plot = linegraph.plot(
    ...
    def parse_fastp_log(self, f):
        ...
        self.fastv_coverage_plotdata[s_name] = {}
        self.fastv_mismatch_plotdata[s_name] = {}
        ...
        if 'genome_mapping_result' in parsed_json:
            ...
            try:
                ...
                log.debug("fastv JSON key missing: '{}'".format(f['fn']))
            try:
                for i, n in enumerate(genome['coverage']):
                    self.fastv_coverage_plotdata[s_name][i+1] = float(n)
            except:
                log.debug("fastv JSON missing coverage result: '{}'".format(f['fn']))
            try:
                for i, n in enumerate(genome['mismatch_ratios']):
                    self.fastv_mismatch_plotdata[s_name][i+1] = float(n)
            except:
                log.debug("fastv JSON missing mismatch ratios: '{}'".format(f['fn']))            
    ...
    def fastp_general_stats_table(self):
        ...
        headers['coverage_rate'] = {
            'title': 'Coverage Rate',
            'description': 'Coverage rate for reference genome',
            'max': 1,
            'min': 0,
            'suffix': '%',
            'modify': lambda x: x * 100.0,
        ...
        headers['avg_mismatch_ratio'] = {
            'title': 'Avg Mismatch Ratio',
            'description': 'Average mismatch ratio to rererence genome',
            'max': 1,
            'min': 0,
            'suffix': '%',
            'modify': lambda x: x * 100.0,
